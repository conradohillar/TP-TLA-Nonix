...
binaryExpression: OPEN_PARENTHESIS expression[left] AND[op] expression[right] CLOSE_PARENTHESIS		{ $$ = BinaryExpressionSemanticAction($left, $right, BINOP_AND); if ($op) { free((void *)$op); }}
	| OPEN_PARENTHESIS expression[left] OR[op] expression[right] CLOSE_PARENTHESIS					{ $$ = BinaryExpressionSemanticAction($left, $right, BINOP_OR);  if ($op) { free((void *)$op); }}
	| OPEN_PARENTHESIS expression[left] THEN[op] expression[right] CLOSE_PARENTHESIS				{ $$ = BinaryExpressionSemanticAction($left, $right, BINOP_THEN); if ($op) { free((void *)$op); }}
	| OPEN_PARENTHESIS expression[left] IFF[op] expression[right] CLOSE_PARENTHESIS					{ $$ = BinaryExpressionSemanticAction($left, $right, BINOP_IFF); if ($op) { free((void *)$op); }}
    ;
...
notExpression: NOT expression 																		{ $$ = NotExpressionSemanticAction($2); if ($1) { free((void *)$1); }} 
	;
%%